...
def absa(model, train_loader, hessian_loader, test_loader, criterion, optimizer, epochs, lr_decay_epoch, lr_decay_ratio, batch_size = 128,
        max_large_ratio = 1, adv_ratio = 0., eps = 0., duration = True, print_flag = False):
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if adv_ratio > 1. / batch_size:
                ...
                adv_data = fgsm(model, data[:adv_r], target[:adv_r], eps)
        ...
        if flag:
            for data, target in hessian_loader:
                ...
                target_eigen = target
                break
            eig, _ = get_eigen(model, data_eigen, target_eigen, criterion, maxIter = 10, tol = 1e-2)
            cur_duration += 1
